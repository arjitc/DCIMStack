...
class WhereClauseTest extends SimpleTest {
    public function test_1_basic_where() {
    ...
    public function test_2_simple_grouping() {
        $where = new WhereClause('and');
        $where->add('password=%s', 'hello');
    ...
    public function test_3_negate_last() {
        $where = new WhereClause('and');
        $where->add('password=%s', 'hello');
        $subclause = $where->addClause('or');
    ...
    public function test_4_negate_last_query() {
        $where = new WhereClause('and');
        $where->add('password=%s', 'hello');
        $subclause = $where->addClause('or');
        $subclause->add('username!=%s', 'Bart');
    ...
    public function test_6_negate_two() {
        $where = new WhereClause('and');
        $where->add('password=%s', 'hello');
        $where->add('username=%s', 'Bart');
        $where->negate();
