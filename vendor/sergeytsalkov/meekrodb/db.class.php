...
class DB {
    ...
    public static function debugMode($enable = true) {
        if ($enable) {
            self::$logfile = fopen('php://output', 'w');
        } else {
            self::$logfile = null;
        }
    }
...
class MeekroDB {
    ...
    public function addHook($type, $fn) {
    ...
    public function removeHook($type, $index) {
        if (! array_key_exists($type, $this->hooks)) {
            throw new MeekroDBException("Hook type $type is not recognized");
        }
    ...
    public function removeHooks($type) {
        if (! array_key_exists($type, $this->hooks)) {
            throw new MeekroDBException("Hook type $type is not recognized");
        }
    ...
    public function runHook($type, $args = array()) {
        if (! array_key_exists($type, $this->hooks)) {
            throw new MeekroDBException("Hook type $type is not recognized");
        }
        ...
        if ($type == 'pre_parse') {
            ...
            return $query;
        } elseif ($type == 'post_run') {
            foreach ($this->hooks[$type] as $hook) {
                call_user_func($hook, $args);
            }
        } elseif ($type == 'run_success') {
            foreach ($this->hooks[$type] as $hook) {
                call_user_func($hook, $args);
            }
        } elseif ($type == 'run_failed') {
            foreach ($this->hooks[$type] as $hook) {
    ...
    public function count() {
        return call_user_func_array(array($this, 'numRows'), func_get_args());
    }
    ...
    public function useDB() {
        return call_user_func_array(array($this, 'setDB'), func_get_args());
    }
    public function setDB($dbName) {
        $db = $this->get();
        if (! $db->select_db($dbName)) {
            throw new MeekroDBException("Unable to set database to $dbName");
        }
    ...
    protected function nextQueryParam($query) {
        ...
        } elseif (substr($query, $first_position_end, $named_seperator_length) == $this->named_param_seperator) {
    ...
    public function debugMode($enable = true) {
        if ($enable) {
            $this->logfile = fopen('php://output', 'w');
        } else {
            $this->logfile = null;
        }
    }
...
class MeekroDBWalk {
    ...
    public function __construct(MySQLi $mysqli, $result) {
        $this->mysqli = $mysqli;
        $this->result = $result;
    }
...
class WhereClause {
    ...
    public function __construct($type) {
        ...
        if ($type !== 'or' && $type !== 'and') {
            throw new MeekroDBException('you must use either WhereClause(and) or WhereClause(or)');
        }
        $this->type = $type;
    }
